...
IMPORTANT: CentOS installation script is still broken and it seems pointless to me to invest more effort. Tell me, if you think it's not! So, only supporting FileMaker Server 19.4 now, as port 16000 is retired now.
...
Open the admin console by clicking the _Open in Browser_ button in the container actions or try with https://localhost/admin-console/signin.
...
Download a copy of the FileMaker Server installer for Linux from your Claris account page, unpack the zip file and move the `.deb` package file to the `build` folder:
...
Open the admin console by clicking the _Open in Browser_ button in the container actions -- if that fails, try with https://localhost/admin-console/signin.
